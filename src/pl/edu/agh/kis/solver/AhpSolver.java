 ...
 * import of data, creating and adding criteria to appropriate CriterionList.
...
public class AhpSolver 
{
	...
	private boolean correctAssociateCriteria()
	{		
		for(int i = 1; i < listOfCriterionList.size(); ++i)
		{
			if(!listOfCriterionList.
				get(i).associateAllCriteria(listOfCriterionList.get(i-1)))
				return false;
		}
	 ...
	 * criteria and alternatives, to end by associate all connections between 
	 * criteria due to informations data.
	 * 
	...
	{
		return correctParseData() && correctAssociateCriteria();
	}
	
	/**
	 * Function lets all criteria from the lowest criterion lawer to the
	...
	public void printFinalResult(boolean showPriorityVector)
	{
		if(showPriorityVector)
			listOfCriterionList.
			get(listOfCriterionList.size()-1).printCriteria();
	...
	/**
	 * Function print all criteria from the lowest layer to the highest.
	 */
