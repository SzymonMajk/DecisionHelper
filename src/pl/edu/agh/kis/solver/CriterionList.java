 ...
 * like layer number informs about type of Criteria, starts by 0 means alternatives
 * criterion and higher number means criteria closer to top. Have several method to
 * cooperate with criteria from cash.
 * 
...
public class CriterionList implements Comparable<CriterionList>
{
	...
	/**
	 * Let all criteria from list, to connect their sub criteria.
	 * 
	 * @param lowerCriteria list with criteria from one level lower layer.
	 * @return true if all connections where succesfuly, false in any other case.
	 */
	public boolean associateAllCriteria(CriterionList lowerCriteria)
	{
		for(Criterion c : criterionCash)
		{
			if(!c.correctAssociateLowerCriteria(lowerCriteria))
				return false;
		}
		return true;
	}
